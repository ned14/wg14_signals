...
> - the `signal` function with the first argument equal to the signal number corresponding to the signal that caused the call of the handler. Furthermore, if such a call to the signal function results in a `SIG_ERR` return, the object designated by `errno` has an indeterminate representation
...
>
> 2. For each thread in a program, an ordered sequence of signal deciders is sequentially invoked to decide how to handle the signal. The ordered sequence begins with thread-locally installed signal deciders with a signal set matching the signal number in order of most recently installed first for that thread, followed by signal deciders installed globally with a signal set matching the signal number:
...
>         - those with `callfirst == true` most recently installed first, and then
>         - those with `callfirst == false` in order of most recently installed last.
>
...
>
> It is permitted for a signal decider to never return. Signal deciders shall meet the same requirements as for a signal handler as specified for the `signal` function (7.14.2.3).
> Decider code which determines that it will never return through the signal decider machinery should call `sigdecider_abandon()` before not returning, and may call `sigdecider_abandon_resume()` to retract that declaration if it later determines that it will return after all (7.14.3.2 and 7.14.3.3).
...
> <ins>
> `SIGGUARDED_FAILURE_VALUE`
> </ins>

> <ins>
> which is a `constexpr` variable of type `union stdc_siginfo_value` with an implementation-defined value, and which is the value returned by `sigguarded()` if it fails to install the guard.
> </ins>
...
>
>   `sigfence()` is *async-signal-safe*. The macro accepts between zero and eight arguments; any additional arguments cause a diagnostic.
...
## Insert the signal set functions into clause 7.14.2 after the introduction, renumbering the subsections below accordingly
...
> For all signals in the signal set `guarded`, the threadsafe implementation shall be activated according to the Introduction above, except that it is implementation-defined which signals are not installed, and for those the signal is silently skipped.
...
> The `raw_info` and `raw_context` members of the `stdc_siginfo` passed to deciders are set from the arguments. If `raw_info` is a null pointer, the `raw_info` member is a null pointer and the `error_code` and `addr` members are zero.
...
> Installs a thread-local signal continuation decider function, recording the current stack and local state such that they can be restored later, after which the recovery function is called, if a decider returns `sig_decision_call_recovery`. See 7.14.1 for how thread-local signal continuation decider functions are called.

> The behavior is undefined if `guarded` or `recovery` does not return, or transfers control out of the function other than by returning, except by abandoning within `decider`.
...
> It is undefined behavior to call this function from within a `recovery` function, or from code which is not currently executing within a decider function.
