...
> Decider code which determines that it will never return through the signal decider machinery should call `sigdecider_abandon()` before not returning, and may call `sigdecider_abandon_resume()` to retract that declaration if it later determines that it will return after all (7.14.4.2 and 7.14.4.3).
...
#### Add 7.14.4.2: The `sigdecider_abandon` function
...
> The `sigdecider_abandon` function declares that the calling code will never return through the signal decider machinery (7.14.1): the `decider` function of the call to `sigguarded` (7.14.4.1), or a globally installed decider function. The argument `rsi` shall be the `stdc_siginfo` pointer passed to the calling decider function; otherwise the behavior is undefined.
...
#### Add 7.14.4.3: The `sigdecider_abandon_resume` function
