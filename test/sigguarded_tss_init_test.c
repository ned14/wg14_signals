...
int main(void)
{
  ...
  // The claiming decider must have been invoked exactly once. On Windows this
  // claim is resolved via EXCEPTION_CONTINUE_EXECUTION (a sigguarded() frame
  // pushes no setjmp frame to longjmp to). The same function is registered as
  // both the unhandled exception filter and the vectored continue handler, and
  // on the no-debugger path Windows invokes both for one exception; the V5
  // dedup marker (analysis.md 3.15) makes the second invocation return the
  // first's recorded decision instead of re-running the deciders, so the
  // claiming decider runs exactly once.
